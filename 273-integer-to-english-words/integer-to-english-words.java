class Solution {
    private static String[] belowTens = {"","One","Two","Three","Four","Five","Six","Seven","Eight","Nine"};
    private static String[] belowTwenty = {"Ten","Eleven","Twelve","Thirteen","Fourteen","Fifteen","Sixteen","Seventeen","Eighteen","Nineteen"};
    private static String[] belowHundred ={"","","Twenty","Thirty","Forty","Fifty","Sixty","Seventy","Eighty","Ninety"};
    ...
    public static String convertToWords(int num)
    {
        if(num == 0)
            return "";
        if(num < 10)
            return belowTens[num];
        
        if(num < 20)
            return belowTwenty[num-10];
        if(num < 100)
            return belowHundred[num/10]+(num%10 !=0 ?" "+belowTens[num%10]:"");
        if(num < 1000)
